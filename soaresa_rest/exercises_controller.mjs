...
//Function to validate the date format (MM-DD-YY)
...
app.post('/exercises', (req, res) => {
    ...
    if(!isDateValid(req.body.date)) {
        res.status(400).json({Error: 'Invalid date formatting'});
        return;
    }

    if(req.body.reps <= 0 || req.body.weight <= 0) {
        res.status(400).json({Error: 'Numbers must be greater than 0'});
        return;
    }
